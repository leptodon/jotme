...
class DatabaseRepositoryImpl(private val db: AppDatabase) : DatabaseRepository {
     ...
     */
    override suspend fun updateInsert(dbNote: DbNote): Unit =
        db.getNotesDao().insertUpdateNote(dbNote)

    /**
     * Получение списка заметок из БД
     */
    override suspend fun getAll(): List<DbNote> =
        db.getNotesDao().gelAll()
